...
class RNNLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.RNN(d_model, d_hidden, n_layer, batch_first=True)
    ...
    def forward(self, idx, targets=None):
        ...
        tok_emb = self.dp_1(tok_emb)
        
        logits, _ = self.lstm(tok_emb) # (B,T,d_hidden)

        #print(logits.dtype)

        logits = self.ff(logits) # (B, T, vocab_size)

        #print(logits.dtype)
        
        logits = self.gelu(self.ln_f(logits)) # (B, T, vocab_size)

        logits = self.dp_2(logits)
